 ...
 */
public class MarkdownFileWalker implements FileVisitor<Path> {
  private ArrayList<MarkdownFile> mdFiles = new ArrayList<>();
  ...
  public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
    if (file.toString().contains(".md")) {
      String path = file.toString();
      String name = path.substring(path.lastIndexOf("/") + 1);
      MarkdownFile mdFile =
          new MarkdownFile(file, name, attrs.creationTime(), attrs.lastModifiedTime());
      mdFiles.add(mdFile);
    }
   ...
   */
  ArrayList<MarkdownFile> getMdFiles() {
    return this.mdFiles;
  }
